...
import com.pp.bayiri.Modif.ModifAub;
...
public class AubAdapter extends RecyclerView.Adapter<AubAdapter.MyHolder> {
    Context context;
    List<HotelClass> list;
    boolean susp= false;

    public AubAdapter(Context context, List<HotelClass> list) {
    ...
    public void onBindViewHolder(@NonNull MyHolder holder, int position) {
        ...
        holder.modif.setOnClickListener(view -> {
            Intent i= new Intent(context, ModifAub.class);
            i.putExtra("c", ""+c);
            context.startActivity(i);
        });

        DatabaseReference reference= FirebaseDatabase.getInstance().getReference()
                .child(Constant.User).child(Constant.location).child("Auberges").child(""+c);

        holder.sup.setOnClickListener(view -> {
                /*Intent i= new Intent(context, SupAub.class);
                i.putExtra("c", ""+c);
                context.startActivity(i); */
            ...
            builder.setNegativeButton("Non", (dialogInterface, i) -> {
                ...
                reference.removeValue().addOnCompleteListener(task -> {
                    if (task.isSuccessful()){
                        Toast.makeText(context, "Auberge supprimée!", Toast.LENGTH_SHORT).show();
                        d.dismiss();
                    }else {
        ...
        holder.suspend.setOnClickListener(view -> {
            ...
            builder.setNegativeButton("Non", (dialogInterface, i) -> {
                    ...
                    .setPositiveButton("Oui", (dialogInterface, i) -> {
                        ProgressDialog d= new ProgressDialog(context);
                        d.setTitle("Suspension en cours...");
                        d.setMessage("Veuillez patienter");
                        d.setCanceledOnTouchOutside(false);
                        d.show();
                                        ...
                                        if (task.isSuccessful()){
                                            reference.removeValue().addOnCompleteListener(task1 -> {
                                                if (task1.isSuccessful()){
                                                    Toast.makeText(context, "Auberge suspendue!", Toast.LENGTH_SHORT).show();
                                                    d.dismiss();
                                                }else {
