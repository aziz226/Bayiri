...
import com.pp.bayiri.Modif.ModifHom;
...
public class ModelAdArticle extends RecyclerView.Adapter<ModelAdArticle.MyHolder> {

    Context context;
    List<AddArticleClass> list;
    boolean susp= false;

    public ModelAdArticle(Context context, List<AddArticleClass> list) {
    ...
    public void onBindViewHolder(@NonNull MyHolder holder, int position) {
        ...
        DatabaseReference refe2 = FirebaseDatabase.getInstance().getReference().child(Constant.User)
                .child(Constant.location).child("Maison").child(c);

        holder.modif.setOnClickListener(view -> {
            Intent i= new Intent(context, ModifHom.class);
            i.putExtra("c", c);
            context.startActivity(i);
        });

        holder.sup.setOnClickListener(view -> {
            /* Intent i= new Intent(context, SupHome.class);
            i.putExtra("c", ""+c);
            context.startActivity(i); */
            ...
            builder.setNegativeButton("Non", (dialogInterface, i) -> {
                ...
                d.show();
                refe2.removeValue().addOnCompleteListener(task -> {
        ...
        holder.suspend.setOnClickListener(view -> {
           /* Intent i= new Intent(context, SupHome.class);
            ...
            DatabaseReference refe1= FirebaseDatabase.getInstance().getReference().child(Constant.User)
                    .child(Constant.susp).child(Constant.location).child("Maison").child(""+c);
            ...
            builder.setNegativeButton("Non", (dialogInterface, i) -> {
                    ...
                    .setPositiveButton("Oui", (dialogInterface, i) -> {
                        ...
                        refe2.addValueEventListener(new ValueEventListener() {
                            @Override
                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                if (snapshot.exists()){
                                    ...
                                    refe1.updateChildren(userMap).addOnCompleteListener(task -> {
                                        if (task.isSuccessful()){
