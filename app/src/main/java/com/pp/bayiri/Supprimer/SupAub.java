...
public class SupAub extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_sup_aub);

        Bundle intent= getIntent().getExtras();
        String cod= intent.getString("c");

        img= findViewById(R.id.sa_img);
        img1= findViewById(R.id.sa_img1);
        img2= findViewById(R.id.sa_img2);
        img3= findViewById(R.id.sa_img3);
        img4= findViewById(R.id.sa_img4);

        loy= findViewById(R.id.sa_hr);
        caution= findViewById(R.id.sa_jr);
        quar= findViewById(R.id.sa_quartier);
        ville= findViewById(R.id.sa_ville);
        code= findViewById(R.id.sa_code_loffr);
        nom= findViewById(R.id.sa_nom);
        sup= findViewById(R.id.sa_sup);
        susp= findViewById(R.id.sa_susp);
        ...
        sup.setOnClickListener(view -> {
            ...
            builder.setTitle("Voulez vous vraiment suprimer cette auberge?");
            ...
            builder.setNegativeButton("Non", (dialogInterface, i) -> {
                        ...
                        userRef.removeValue().addOnCompleteListener(task -> {
                            if (task.isSuccessful()){
                                Toast.makeText(this, "Auberge supprimée!", Toast.LENGTH_SHORT).show();
        ...
        susp.setOnClickListener(view -> {
            DatabaseReference reference= FirebaseDatabase.getInstance().getReference().child(Constant.User)
                    .child(Constant.susp).child(Constant.location).child("Auberges").child(""+cod);

            AlertDialog.Builder builder= new AlertDialog.Builder(this);
            builder.setTitle("Voulez vous vraiment suspendre cette auberge?");
            builder.setMessage("Une fois suspendue, cette auberge ne sera plus visible pour les utilisateurs...");
            builder.setNegativeButton("Non", (dialogInterface, i) -> {
                                        ...
                                        if (task.isSuccessful()){
                                            userRef.removeValue().addOnCompleteListener(task1 -> {
                                                if (task1.isSuccessful()){
                                                    Toast.makeText(SupAub.this, "Auberge suspendue!", Toast.LENGTH_SHORT).show();
                                                    startActivity(new Intent(SupAub.this, AddLocatHouse.class));
                                                    d.dismiss();
                                                }else {
                                                    Toast.makeText(SupAub.this, "Erreur: "+task1.getException().getMessage(),
                                                            Toast.LENGTH_SHORT).show();
                                                    d.dismiss();
                                                }
                                            });
                                        }else {
                                            Toast.makeText(SupAub.this, "Erreur: "+task.getException().getMessage(),
                                                    Toast.LENGTH_SHORT).show();
                                            d.dismiss();
                                        }
                                    });
