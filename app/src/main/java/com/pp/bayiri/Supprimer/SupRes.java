...
public class SupRes extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_sup_res);

        Bundle intent= getIntent().getExtras();
        String cod= intent.getString("c");

        img= findViewById(R.id.sr_img);
        img1= findViewById(R.id.sr_img1);
        img2= findViewById(R.id.sr_img2);
        img3= findViewById(R.id.sr_img3);
        img4= findViewById(R.id.sr_img4);

        d1= findViewById(R.id.sr_des1);
        d2= findViewById(R.id.sr_des2);
        d3= findViewById(R.id.sr_des3);
        d4= findViewById(R.id.sr_des4);
        d5= findViewById(R.id.sr_des5);
        d6= findViewById(R.id.sr_des6);
        d7= findViewById(R.id.sr_des7);
        d8= findViewById(R.id.sr_des8);
        d9= findViewById(R.id.sr_des9);
        d10= findViewById(R.id.sr_des10);

        loy= findViewById(R.id.sr_loyer);
        caution= findViewById(R.id.sr_caution);
        obli= findViewById(R.id.sr_obli);
        obli_pri= findViewById(R.id.sr_obli_pri);
        code= findViewById(R.id.sr_code);
        sup= findViewById(R.id.sr_sup);
        susp= findViewById(R.id.sr_susp);

        userRef= FirebaseDatabase.getInstance().getReference()
                .child(Constant.User).child(Constant.location).child("Residences").child(""+cod);
            ...
            builder.setNegativeButton("Non", (dialogInterface, i) -> {
                ...
                userRef.removeValue().addOnCompleteListener(task -> {
                    if (task.isSuccessful()){
                        Toast.makeText(this, "Résidence supprimée!", Toast.LENGTH_SHORT).show();
        ...
        susp.setOnClickListener(view -> {
            DatabaseReference reference= FirebaseDatabase.getInstance().getReference().child(Constant.User)
                    .child(Constant.susp).child(Constant.location).child("Residences").child(""+cod);
                                ...
                                if (task.isSuccessful()){
                                    userRef.removeValue().addOnCompleteListener(task1 -> {
                                        if (task1.isSuccessful()){
                                            Toast.makeText(SupRes.this, "Maison supprimée!", Toast.LENGTH_SHORT).show();
                                            startActivity(new Intent(SupRes.this, AddLocatHouse.class));
                                            d.dismiss();
                                        }else {
                                            Toast.makeText(SupRes.this, "Erreur: "+task1.getException().getMessage(),
                                                    Toast.LENGTH_SHORT).show();
                                            d.dismiss();
                                        }
                                    });
                                }else {
                                    Toast.makeText(SupRes.this, "Erreur: "+task.getException().getMessage(),
                                            Toast.LENGTH_SHORT).show();
                                    d.dismiss();
                                }
                            });
