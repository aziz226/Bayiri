...
public class HmAdapter extends RecyclerView.Adapter<HmAdapter.MyHolder> {

    Context context;
    List<AddArticleClass> list;

    public HmAdapter(Context context, List<AddArticleClass> list) {
    ...
    public void onBindViewHolder(@NonNull MyHolder holder, int position) {
        ...
        DatabaseReference reference2 = FirebaseDatabase.getInstance().getReference().child(Constant.User)
       ...
       DatabaseReference reference= FirebaseDatabase.getInstance().getReference().child(Constant.User)
        ...
        DatabaseReference userRef= FirebaseDatabase.getInstance().getReference()
                .child(Constant.User).child(Constant.location).child("Maison").child(""+c);
        holder.suspend.setOnClickListener(view -> {
            ...
            builder.setTitle("Voulez vous vraiment rendre cette maison disponile?");
            builder.setMessage("Une fois disponible, cette maison ne sera visible pour les utilisateurs...");
                                ...
                                if (snapshot.exists()){
                                    ...
                                    userRef.updateChildren(userMap).addOnCompleteListener(task -> {
                                        if (task.isSuccessful()){
                                           ...
                                           // Intent intent= new Intent(context, HomeSusp.class);
            ...
            /* reference1.addValueEventListener(new ValueEventListener() {
                            ...
                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                if (snapshot.exists()){
                                    ...
                                    reference2.updateChildren(userMap).addOnCompleteListener(task -> {
                                        if (task.isSuccessful()){
                                            if (task.isSuccessful()){
                                                Toast.makeText(context, "Maison disponible!", Toast.LENGTH_SHORT).show();
