...
public class HoAdapter extends RecyclerView.Adapter<HoAdapter.MyHolder> {
    Context context;
    List<HotelClass> list;

    public HoAdapter(Context context, List<HotelClass> list) {
    ...
    public void onBindViewHolder(@NonNull MyHolder holder, int position) {
        ...
        holder.suspend.setOnClickListener(view -> {
            ...
            builder.setTitle("Voulez vous vraiment rendre disponible cette hotel?");
            builder.setMessage("Une fois rendu disponible, cette hotel sera visible pour les utilisateurs...");
                                ...
                                if (snapshot.exists()){
                                   ...
                                   // userMap.put("periode", snapshot.child("periode").getValue().toString());
                                    ...
                                    reference.updateChildren(userMap).addOnCompleteListener(task -> {
                                        Toast.makeText(context, "Hotel mise en ligne !", Toast.LENGTH_SHORT).show();
                                        if (task.isSuccessful()) {
                                            userRef.removeValue().addOnCompleteListener(task1 -> {
                                                if (task1.isSuccessful()) {
                                                    Intent intent = new Intent(context, HotelSusp.class);
                                   ...
                                   /* reference.updateChildren(userMap).addOnCompleteListener(task -> {
                                        if (task.isSuccessful()){
                                            Toast.makeText(context, "hotel disponible!", Toast.LENGTH_SHORT).show();
