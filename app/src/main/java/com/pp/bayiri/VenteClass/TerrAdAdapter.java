...
import com.pp.bayiri.Modif.ModifTerrain;
...
public class TerrAdAdapter extends RecyclerView.Adapter<TerrAdAdapter.MyHolder> {

    Context context;
    List<TmClass> list;

    public TerrAdAdapter(Context context, List<TmClass> list) {
    ...
    public void onBindViewHolder(@NonNull MyHolder holder, int position) {
        ...
        holder.modif.setOnClickListener(view -> {
            Intent i= new Intent(context, ModifTerrain.class);
            i.putExtra("c", ""+c);
        ...
        holder.suspend.setOnClickListener(view -> {
            DatabaseReference reference= FirebaseDatabase.getInstance().getReference().child(Constant.User)
                    .child(Constant.susp).child(Constant.vente).child("Terrain").child(""+c);
                                        ...
                                        if (task.isSuccessful()){
                                            userRef.removeValue().addOnCompleteListener(task1 -> {
                                                if (task1.isSuccessful()){
                                                    ...
                                                    Intent intent= new Intent(context, AjouterVente.class);
